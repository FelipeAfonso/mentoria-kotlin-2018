...
class DisciplinaBDHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun inserirDisciplina(disc : Disciplina): Boolean {
        ...
        //Linha abaixo comentada porque APK será Auto Increment
        //values.put(DBContract.Disciplinas_Ent.COL_DISC_ID, disc.id)
    ...
    fun lerTodasDisciplinas(): ArrayList<Disciplina> {
        ...
        try {
            ...
            cursor = db.rawQuery("select * from " + DBContract.Disciplinas_Ent.TABLE_NAME + "ORDER BY " + DBContract.Disciplinas_Ent.COL_DISC_ID, null)
        } catch (e: SQLiteException) {
            db.execSQL(SQL_CREATE_ENTRIES)
            return ArrayList()
        }
    ...
    companion object {
        ...
        private val SQL_CREATE_ENTRIES =
                "CREATE TABLE " + DBContract.Disciplinas_Ent.TABLE_NAME + " (" +
                        ...
                        DBContract.Disciplinas_Ent.COL_NOTA + " REAL);"
...
////////////// SE DER ERRO TUDO PARA TEXT E STRING
