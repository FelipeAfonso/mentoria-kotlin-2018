...
class DisciplinaBDHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    @Throws(SQLiteConstraintException::class)
    fun editarDisciplina(disciplina: Disciplina) : Boolean {
        val db = writableDatabase
        val values = ContentValues()

        values.put(DBContract.Disciplinas_Ent.COL_SIGLA, disciplina.sigla)
        values.put(DBContract.Disciplinas_Ent.COL_NOME, disciplina.nome)
        values.put(DBContract.Disciplinas_Ent.COL_SEM, disciplina.semAno)
        values.put(DBContract.Disciplinas_Ent.COL_NOTA, disciplina.nota)
        db.update(DBContract.Disciplinas_Ent.TABLE_NAME, values, DBContract.Disciplinas_Ent.COL_DISC_ID + "=" + disciplina.id, null)
        return true
    }
